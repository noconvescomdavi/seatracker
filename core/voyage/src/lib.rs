...
impl VoyageLeg {
    pub fn ttg_hours(&self) -> Option<f64> {
        if self.speed_knots <= 0.0 { None } else { Some(self.distance_nm / self.speed_knots) }
    }
}
...
pub fn total_ttg_hours(legs: &[VoyageLeg]) -> Option<f64> {
    let mut total=0.0;
    for leg in legs { total += leg.ttg_hours()?; }
    Some(total)
}
...
mod tests {
    use super::*;
    #[test] fn voyage_totals() {
        let l=VoyageLeg{from:"A".into(),to:"B".into(),course_deg:90.0,distance_nm:20.0,speed_knots:10.0};
