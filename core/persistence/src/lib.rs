...
pub fn put_json<T: Serialize>(store: &mut dyn KeyValueStore, key: &str, value: &T) -> Result<(), String> {
    let data = serde_json::to_vec(value).map_err(|e| e.to_string())?;
    store.put_bytes(key, &data)
}

pub fn get_json<T: DeserializeOwned>(store: &dyn KeyValueStore, key: &str) -> Result<Option<T>, String> {
    let Some(data)=store.get_bytes(key)? else { return Ok(None) };
