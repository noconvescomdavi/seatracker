...
pub fn meters_to_nm(meters: f64) -> f64 { meters / METERS_PER_NAUTICAL_MILE }
pub fn nm_to_meters(nm: f64) -> f64 { nm * METERS_PER_NAUTICAL_MILE }
...
pub fn destination_point(lat:f64, lon:f64, bearing_deg:f64, distance_m:f64) -> (f64,f64) {
    ...
    let lambda2=lambda1+(theta.sin()*delta.sin()*phi1.cos())
...
pub fn rhumb_distance_m(lat1:f64, lon1:f64, lat2:f64, lon2:f64) -> f64 {
    ...
    if dl.abs()>std::f64::consts::PI { dl=if dl>0.0 {-(2.0*std::f64::consts::PI-dl)} else {2.0*std::f64::consts::PI+dl}; }
    let dpsi=((p2/2.0+std::f64::consts::FRAC_PI_4).tan()/(p1/2.0+std::f64::consts::FRAC_PI_4).tan()).ln();
...
pub fn ttg_seconds(distance_m:f64, speed_knots:f64) -> Option<f64> {
    if !distance_m.is_finite() || !speed_knots.is_finite() || speed_knots<=0.0 { return None; }
...
pub fn required_speed_knots(distance_m:f64, available_seconds:f64) -> Option<f64> {
    if distance_m<0.0 || available_seconds<=0.0 { return None; }
...
mod tests {
    use super::*;
    #[test] fn nm_roundtrip(){assert!((meters_to_nm(nm_to_meters(12.3))-12.3).abs()<1e-12);}
    #[test] fn longitude_wrap(){assert_eq!(normalize_longitude(190.0),-170.0);}
    #[test] fn known_equator_distance(){let d=great_circle_distance_m(0.0,0.0,0.0,1.0);assert!((meters_to_nm(d)-60.04).abs()<0.2);}
    #[test] fn destination_roundtrip(){let p=destination_point(0.0,0.0,90.0,nm_to_meters(60.0));assert!(p.0.abs()<0.01);assert!((p.1-0.999).abs()<0.02);}
    #[test] fn ttg(){assert_eq!(ttg_seconds(nm_to_meters(10.0),10.0).map(|v|v.round()),Some(3600.0));}
}
